...
def generate_synthetic_square(im_dims, square_dims, rgb=True):
    ...
    square = 200*np.ones(square_dims, dtype=np.uint8)
    if rgb is True:
        im = np.random.randint(0, 55, (im_dims[0], im_dims[1], 3), dtype=np.uint8)
        ...
        im = np.random.randint(0, 55, (im_dims[0], im_dims[1]), dtype=np.uint8)
        im[im_dims[0]//2 - square_dims[0]//2:im_dims[0]//2 + square_dims[0]//2,
        im_dims[1]//2 - square_dims[1]//2:im_dims[1]//2 + square_dims[1]//2] += square
    
    ground_truth = np.ones(im_dims, dtype=np.uint8)
    ground_truth[im_dims[0]//2 - square_dims[0]//2:im_dims[0]//2 + square_dims[0]//2,
                    im_dims[1]//2 - square_dims[1]//2:im_dims[1]//2 + square_dims[1]//2] = 2
