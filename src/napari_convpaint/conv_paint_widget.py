...
### Define the basic structure of the widget
    ...
    def __init__(self, napari_viewer, parent=None, init_project=False, third_party=False):
        ...
        self.set_default_clfbtn = QPushButton('Restore defaults')
        self.set_default_clfbtn.setEnabled(True)
        self.classifier_params_group.glayout.addWidget(self.set_default_clfbtn, 3, 0, 1, 2)
    ...
    def add_connections(self):
        ...
        # Classifier
        self.spin_iterations.valueChanged.connect(lambda: setattr(self.param, 'clfiterations', self.spin_iterations.value()))
        self.spin_learning_rate.valueChanged.connect(lambda: setattr(self.param, 'clflearning_rate', self.spin_learning_rate.value()))
        self.spin_depth.valueChanged.connect(lambda: setattr(self.param, 'clfdepth', self.spin_depth.value()))
        self.set_default_clfbtn.clicked.connect(self._on_reset_clfparams)
...
### Define the detailed behaviour of the widget
    ...
    def _on_train(self):
        ...
        with progress(total=0) as pbr:
            ...
            self.classifier = train_classifier(
                features, targets,
                iterations=self.param.clfiterations,
                learning_rate=self.param.clflearning_rate,
                depth=self.param.clfdepth,
        )
    ...
    def _on_fe_scalings_changed(self):
        """Only update param object, when FE is set."""
        return
    ...
    def _on_set_fe_model(self, event=None):
        ...
        val_to_setter = {
            ...
            "clfiterations": self.spin_iterations.setValue,
            "clflearning_rate": self.spin_learning_rate.setValue,
            "clfdepth": self.spin_depth.setValue
        }
        for attr, setter in val_to_setter.items():
            val = getattr(default_param, attr, None)
            if val is not None:
...
### Helper functions
    ...
    def _reset_clfparams(self):
        ...
        self.param.clfiterations = self.DEFAULT_ITERATIONS
        self.param.clflearning_rate = self.DEFAULT_LEARNING_RATE
        self.param.clfdepth = self.DEFAULT_DEPTH
